...
/*         if ((grid.x(gi) < XCORNER && grid.y(gj) >= YCORNER) || (grid.x(gi) >= block_x1 && grid.x(gi) <= block_x2 && grid.y(gj) >= block_y1 && grid.y(gj) <= block_y2)) {
*/
    ...
    if (plotVariable >= DISPLAYTYPES) {
      ...
      data[ni * j + i] = exp(-0.10 * sqrt(drhodx * drhodx + drhody * drhody));
